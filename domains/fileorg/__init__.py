import argparse
from domains.fileorg.verbs.diff import run_diff_mode
from domains.fileorg.verbs.sync import run_sync_mode
from domains.fileorg.verbs.merge import run_merge_mode
...
def register_cli(subparsers):
    ...
    p_diff.add_argument("-p", "--paths-file", default="~/.dcomplib/paths.json", help="Paths registry JSON file.")
    ...
    p_sync.add_argument("-p", "--paths-file", default="~/.dcomplib/paths.json", help="Paths registry JSON file.")
...
def mount_cli(subparsers):
    ...
    import sys
    from domains.core.cliux.noun import discover_cli_nouns
        ...
        for name, flow in workflows.items():
            ...
            try:
                ...
                if generated_workflows and hasattr(generated_workflows, func_name):
                    ...
                    def make_fallback(n):
                        def fallback(args):
                            print(f"Error: Workflow '{n}' is not compiled. Run 'combinate domain compile-workflows fileorg'.")
                        return fallback
