...
def register_cli(subparsers):
    ...
    p_detect.add_argument("-p", "--paths-file", default="~/.dcomplib/paths.json", help="Paths registry JSON file.")
...
def run_detect_verb(args):
    ...
    from dcomplib.io import load_and_merge_scans, save_scan_data, atomic_write_json
    from .analyzer import analyze_scenes
    
    print(f"--- Running Scene Detection Mode ---")
    scan_files = getattr(args, 'scan_files', ["cache.json"])
    paths_file = getattr(args, 'paths_file', "~/.dcomplib/paths.json")
...
def run_prune_verb(args):
    ...
    if data_was_modified and not args.dry_run:
        save_scan_data(args.scan_files, context, True, getattr(args, 'paths_file', None) or '~/.dcomplib/paths.json', getattr(args, 'metadata_file', None) or 'metadata.json')
