...
from dcomplib.combinators import Pipeline, Load, Filter, Rule
from dcomplib.entities import Noun
from typing import Any, Dict, List

from dcomplib.combinators import Pipeline, Load, Filter, Rule, Combinator

class Resolve(Combinator):
    """Pipe: Resolves aliased paths in a stream to absolute physical paths."""
    ...
    def __call__(self, data):
        ...
        if not self.context:
            ...
            self.context = load_and_merge_scans([])
        
        if isinstance(data, dict):
            return {resolve_token_path(self.context, k): v for k, v in data.items()}
        return [resolve_token_path(self.context, x) for x in data]

class Alias(Combinator):
    """Pipe: Converts physical paths in a stream into PATHxx tokens."""
    def __init__(self, context=None):
        self.context = context
    def __call__(self, data):
        if not self.context:
        ...
        # Helper to find token for a physical path
        def to_token(p):
            for token, info in self.context.paths.items():
                if info.get('mount') == p: return token
            return p

        if isinstance(data, dict):
            return {to_token(k): v for k, v in data.items()}
        return [to_token(x) for x in data]
...
def register_cli(subparsers):
    ...
    # Verb: resolve
    p_resolve = path_sub.add_parser("resolve", help="Resolve a aliased path to a physical path.")
    ...
    # Verb: alias
    p_alias = path_sub.add_parser("alias", help="Find or create a token for a physical path.")
    p_alias.add_argument("path", help="The absolute physical path.")
    p_alias.add_argument("-s", "--scan-files", default=["cache.json"], nargs='+', help="Scan/cache files.")
    p_alias.add_argument("--create", action='store_true', help="Create a new token if not found.")
    p_alias.add_argument("--save", action='store_true', help="Save changes to cache.")
    p_alias.add_argument("--no-uuid-only", dest='uuid_only', action='store_false', help="Disable requiring UUID.")
    p_alias.set_defaults(func=run_alias_verb, uuid_only=True)
    ...
    p_get.add_argument("token_path", help="The aliased path to the file.")
...
def run_resolve_verb(args):
    from dcomplib import load_and_merge_scans, resolve_token_path
    context = load_and_merge_scans(args.scan_files)
    print(resolve_token_path(context, args.token_path))

def run_alias_verb(args):
    from dcomplib import load_and_merge_scans, get_or_create_path_token, save_scan_data
    context = load_and_merge_scans(args.scan_files)
    raw = args.path
    try:
...
def run_update_verb(args):
    from dcomplib import load_and_merge_scans, save_scan_data
    context = load_and_merge_scans(args.scan_files)
...
def query_pipeline(args):
    ...
    """
    return Pipeline([
        Load(getattr(args, 'paths_file', '~/.dcomplib/paths.json'), 'paths'),
