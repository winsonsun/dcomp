...
def _resolve_dido(dido_path: str):
    ...
    if len(path_parts) == 3:
        ...
        module_path = f"domains.{domain}.{noun}.noun"
    elif len(path_parts) == 2:
        # Cross-cutting domain verb (e.g. @fileorg.diff)
        domain, _ = path_parts
        module_path = f"domains.{domain}.verbs.{func_name}"
    else:
        module_path = ".".join(["domains"] + path_parts[:-1])
...
def mount_domain_commands(blueprint: dict, subparsers: argparse._SubParsersAction):
    ...
    for noun_name, actions in commands.items():
        # First, try to get or create the noun-level parser (e.g., 'dcomplib scene')
        ...
        # For this prototype, we'll mount them as 'dcomplib [noun] [action]' directly.
