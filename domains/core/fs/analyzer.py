...
def run_scan_mode(args):
    ...
    """
    from dcomplib import load_jobs_file, load_and_merge_scans, save_scan_data, save_jobs_config, ensure_path_mappings, get_or_create_path_token
    from dcomplib.combinators import Pipeline, FS_Scan, Map, BuildTree, Rule, Filter
    from dcomplib.io import IMAGE_EXTENSIONS, VIDEO_EXTENSIONS
    import dcomplib.policy as policy
    ...
    context = load_and_merge_scans(args.scan_files, getattr(args, 'paths_file', None) or '~/.dcomplib/paths.json', getattr(args, 'metadata_file', None) or 'metadata.json')
        ...
        for dir_key in sorted(job.keys()):
            ...
            try:
                ...
                def apply_tokens(item):
                    ...
                    aliased_path = f"{token}/{rel_clean}" if rel_clean else f"{token}/"
                    
                    new_props = dict(props)
                    new_props['full_path'] = aliased_path
                    # Pass rel path along for tree building
                    new_props['_rel'] = rel
                    return (aliased_path, new_props)
                ...
                alias_pipeline = Pipeline(token_steps)
                aliased_list = alias_pipeline.execute(raw_items)
                
                # Unpack
                if isinstance(aliased_list, dict):
                    aliased_list = list(aliased_list.values())
                    
                aliased_items_map = {props['_rel']: props for t_path, props in aliased_list}
                aliased_db_items = {t_path: props for t_path, props in aliased_list}
                
                file_count = sum(1 for props in aliased_db_items.values() if props.get('type') == 'file')
                ...
                for t_path, props in aliased_db_items.items():
                ...
                rel_map = {props['full_path']: props.get('_rel') for props in aliased_items_map.values()}
                ...
                job_tree = tree_pipeline.execute(aliased_items_map)
                
                for props in aliased_items_map.values():
                    props.pop('_rel', None)
                for props in aliased_db_items.values():
                    props.pop('_rel', None)

                context.database["items"].update(aliased_db_items)
    ...
    try:
        ...
        save_scan_data(args.scan_files, save_context, save_needed, getattr(args, 'paths_file', None) or '~/.dcomplib/paths.json', getattr(args, 'metadata_file', None) or 'metadata.json')
        save_jobs_config(args.job_file, jobs_config, job_config_was_modified)

        from dcomplib.io import atomic_write_json
