...
* **Debuggability**: If a path fails to alias, developers know exactly where to look (`modes.py`), rather than tracing through a dozen dynamically registered plugins.
...
1. Keep the core pipeline steps (`FS_Scan`, `Map(alias)`, `BuildTree`) hardcoded in `modes.py`.
2. Provide explicit extension points (e.g., `pre_scan`, `post_alias`) where `policy.get_rules` is queried.
...
* `paths.py` would inject the `Map(alias)` combinator at priority `20`.
