...
class Location:
    ...
    """
    def __init__(self, local_paths_file="~/.dcomp/paths.json", remote_paths_data=None):
...
class SyncManifest:
    ...
    def __call__(self, diff_result):
        ...
        for item in diff_result.get('only_in_dir1', []):
            ...
            intents.append({
                "action": "COPY",
                "source": item.get('full_path', item.get('full_path1')), # Support live FS paths and tokenized ones
                "target_rel": rel_path,
                "status": "pending"
            })
