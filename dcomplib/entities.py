...
class EntityResolver:
    ...
    def __init__(self, media_cache_files=None, metadata_file="metadata.json", paths_file="~/.dcomplib/paths.json", jobs_file="jobs.json", context=None):
    ...
    def get_noun_module(self, target_uri: str) -> Any:
        ...
        namespaces = ["domains.core", "domains.fileorg"]
        user_domains = os.environ.get("DCOMP_USER_DOMAIN")
        if user_domains:
            for d in user_domains.split(';'):
                d = d.strip()
                if d and not d.startswith("domains."): d = f"domains.{d}"
                if d and d not in namespaces: namespaces.append(d)
    ...
    def resolve_for_diff(self, target_uri):
        ...
        from dcomplib.contracts import Diffable
        for ns in namespaces:
            module_path = f"{ns}.{noun}.noun"
            try:
                noun_module = importlib.import_module(module_path)
