## 1. The Isolated Workspace (ext)
All of your work MUST be contained within the `ext` directory.
*   **The Guardrail:** If you are asked to "fix a bug" or "add a field" to a core noun, DO NOT modify the file in `core`. Instead, create a **Shadow Noun** or a **Policy Extension** in `ext`.
...
*   **The 'Why' Tracker:** When recording architectural decisions or filling out the `ai_ontology`, you must anchor the reasoning to the **Data Identity** and the **Workflow**, not the implementation details. (e.g., Record 'We filter `.tmp` files here to preserve downstream Cache purity', do NOT record 'We used a Python generator here to save memory').
