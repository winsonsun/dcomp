...
def _resolve_dido(dido_path: str):
    ...
    # For now, assume the module structure maps to dcomp.<domain>.<noun>.noun
    # e.g., @fileorg.scene.detect -> dcomp.fileorg.scene.noun.detect
    if len(path_parts) == 3:
        domain, noun, _ = path_parts
        module_path = f"dcomp.{domain}.{noun}.noun"
    elif len(path_parts) == 2:
        # Cross-cutting domain verb (e.g. @fileorg.diff)
        domain, _ = path_parts
        module_path = f"dcomp.{domain}.verbs.{func_name}"
    else:
        module_path = ".".join(["dcomp"] + path_parts[:-1])
...
def discover_cli_nouns(provides_context: list):
    ...
    """
    from dcomp.combinators import Pipeline, Filter, Rule, Map
