...
class Resolve(Combinator):
    ...
    def __call__(self, data):
        from dcomp.store import resolve_token_path
        if not self.context:
...
def run_resolve_verb(args):
    from dcomp import load_and_merge_scans, resolve_token_path
    context = load_and_merge_scans(args.scan_files)
    print(resolve_token_path(context, args.token_path))

def run_alias_verb(args):
    from dcomp import load_and_merge_scans, get_or_create_path_token, save_scan_data
    context = load_and_merge_scans(args.scan_files)
    raw = args.path
    try:
