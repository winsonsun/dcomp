...
def run_execute_verb(args):
    ...
    def handle_snapshot(d, baseline):
        import subprocess
        print(f"  [Air-Gap] Spawning clean runtime to snapshot state...")
        subprocess.run(
            ["python3", "combinate.py", "domain", "snapshot", baseline, "-s", "cache.json"],
            check=True
        )
    ...
    def handle_wire_pipeline(d):
        from meta.pdm.evolve import OntologyEvolver
        import re
        ...
        evolver = OntologyEvolver("wire_pipeline execution")
        code = evolver.run_llm(prompt, sys_prompt)
    ...
    def handle_verify(d, baseline):
        import subprocess
        print(f"  [Air-Gap] Spawning clean runtime to verify state against snapshot...")
        result = subprocess.run(
            ["python3", "combinate.py", "domain", "verify", "--snapshot", baseline, "-s", "cache.json"],
